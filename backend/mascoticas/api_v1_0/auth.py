    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            if current_user is None:
                return {
                "message": "Token invalido! vuelva a iniciar sesion",
                "data": None,
                "error": "Unauthorized"
            }, 401
        except Exception as e:
            return {
                "message": "Algo ocurrio! intente de nuevo",
                "data": None,
                "error": str(e)
            }, 500
...
class UserLoginResource(Resource):
    ...
    def put(self):
        '''Cambio de password
        Entrada: body: correo,telefono,new_password
        salida: json(str), 201 o json(str),401/500'''
        put_data = request.get_json()
        usuario = Usuario.simple_filter(correo = put_data.get('correo'),telefono = put_data.get('telefono'))[0]
        print(usuario)
        if usuario:
            try:
                ...
                response_object = {
                    'status': 'success',
                    'message': 'Usuario habilitado, cambio de password aceptado',
                    'auth_token': user_token
                }
                return response_object, 201
            except Exception as e:
